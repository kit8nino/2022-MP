...
max_x = len(maze[0]) # определяем ширину
max_y = len(maze) # определеяем высоту
    
POSSIBLE_WAYS = ['U', 'D', 'R', 'L'] # определяем направления движения из конкретной точки
coord_treasure = [0, 0] # куда нам нужно дойти


# Описываем движение точки до места назначения
...
def step_up(coord): # верх
    return [coord[0], coord[1]-1]
def step_right(coord): # право
    return [coord[0]+1, coord[1]]
def step_down(coord): # низ
    return [coord[0], coord[1]+1]
def step_left(coord): # лево
    return [coord[0]-1, coord[1]]

def cut_way_back(direction): # блокировка ненужного движения в обратную сторону
    if direction == 'U':
        return ('U', 'R', 'L')
    if direction == 'D':
        return ('D', 'R', 'L')
...
# работа с файлом и его содержимым
...
def enter_coord_of_treasure():
    q = 0
    while q == 0:
        coord_treasure[0] = int(input(f'Choose start x is between 1 and {max_x} ')) - 1 
        coord_treasure[1] = int(input(f'Choose start y is between 1 and {max_y} ')) - 1
        if maze[coord_treasure[1]][coord_treasure[0]] == '#':
            print('Error! Choose pt again!')
        else:
...
print("Open the file and enjoy it!")
