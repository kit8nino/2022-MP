...
def step_up(coord): # вверх
    return [coord[0], coord[1]-1]
def step_right(coord): # направо
    return [coord[0]+1, coord[1]]
def step_down(coord): # вниз
    return [coord[0], coord[1]+1]
def step_left(coord): # влево
    return [coord[0]-1, coord[1]]
...
def enter_coord_of_treasure():
    q = 0
    while q == 0:
        coord_treasure[0] = int(input(f'Где сокровище? (x между 1 и {max_x}) ')) - 1 
        coord_treasure[1] = int(input(f'Где сокровище? (y между 1 и {max_y}) ')) - 1
        if maze[coord_treasure[1]][coord_treasure[0]] == '#':
            print('Ошибка! Здесь нет сокровища! Здесь стена!')
        else:
...
def mark_the_path(maze_list, coord, path_to_exit): # нарисовать путь в файле
    for i in path_to_exit:
        change_the_symbol(maze_list, coord, '.') # точки на маршруте
