...
while(not (treasure_is_here and is_path_clean(maze, treasure_is_here))):
    x = np.random.randint(5,100) #int(input(f'Where is threasure? (x is between 0 and {max_x-1}) '))
    y = np.random.randint(5,200) #int(input(f'Where is threasure? (y is between 0 and {max_y-1}) '))
    x=67
    y=27
