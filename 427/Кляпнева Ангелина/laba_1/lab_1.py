...
def maze_open():
    ...
    with open('maze-for-u.txt') as mazefu:
        for line in mazefu:
           maze.append(line.strip('\n'))
        x_max, y_max = len(maze[0]), len(maze)
    return x_max,y_max, maze

max_x,max_y,maze=maze_open()
maze_list= list(maze)
...
#ищем координаты первой пустой точки в ряду
...
#путь до сокровища от точки coord
...
#поиск стартовой и конечной точек
...
#чтобы сокровище не осталось в стене
