...
max_x, max_y = len(maze[0]), len(maze) #Ширина и высота

# =============================================================================
# Movement
# =============================================================================

POSSIBLE_WAYS = ['N', 'S', 'E', 'W'] #Возможные способы передвижения
...
def is_coord_in_maze(maze, coord): #Проверка выхода за пределы лабиринта
...
def is_path_clean(maze, coord): #Проверка на наличие стены в пути
    if maze[coord[1]][coord[0]] == '#':
        return False
    return True

coord_treasure = [0, 0] #Переменная для записи координат сокровища
def input_coord_of_treasure(): #Постановка сокровища и предупреждение о постановки в стену
    check = False
    while check == False:
...
def is_coord_treasure(coord): #Проверка положения сокровища
    global treasure_is_here
    if(coord[1] == treasure_is_here[1] and coord[0] == treasure_is_here[0]):
        return True
    return False

def replace_symbol(maze_list, coord, symbol): #Замена символа
    maze_row = list(maze_list[coord[1]])
    maze_row[coord[0]] = symbol
    maze_list[coord[1]] = ''.join(maze_row)
    
def find_empty_symbol(maze, i): #Поиск пустого символа в строке
...
def tag_path(maze_list, coord, path_to_exit): # нарисовать путь в файле
    for i in path_to_exit:
        replace_symbol(maze_list, coord, '.') # проставление точек на всем маршруте
...
replace_symbol(maze_list, treasure_is_here, '*')#Установка сокровища

find_a_way(maze, start_point, POSSIBLE_WAYS)#Нахождение и пометка пути до сокровища
tag_path(maze_list, start_point, path_to_exit)#

exit_point = find_empty_symbol(maze, max_y - 1)#Поиск точки выхода

find_the_exit(treasure_is_here, exit_point)#Поиск к выходу от сокровища
