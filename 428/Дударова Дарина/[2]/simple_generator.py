...
class Generator_simple:
    ...
    def __init__(self, typeSignal: str = "Harmonic" , parametr : float = 1.):
        ...
        if typeSignal == "Harmonic":
            ...
            self.function_signal = lambda x: float(x) if float(x) < 0.5 else                float(1 - x) if float(x < 1.5) else float(x - 2.)
        elif typeSignal == "PWM":
    ...
    def getSignalForTime(self , t : float):
        return {t : self.function_signal((t/self.period) % 5)*self.amplitude}
