...
POSSIBLE_WAYS = ['U', 'D', 'R', 'L'] # Способы передвижения
coord_treasure = [0, 0] # Координаты сокровища


# Движения кладоискателя
...
def cut_way_back(direction): # Невозможность возвращаться назад 
    if direction == 'U':
        return ('U', 'R', 'L')
    if direction == 'D':
        return ('D', 'R', 'L')
...
def enter_coord_of_treasure():
    ...
    while q == 0:
        ...
        if maze[coord_treasure[1]][coord_treasure[0]] == '#':
            print('Error! There can be no treasure!')
        else:
            treasure_is_here = (abs(coord_treasure[0]) if abs(coord_treasure[0]) < max_x-1 else max_x-1,\
                                abs(coord_treasure[1]) if abs(coord_treasure[1]) < max_y-1 else max_y-1)
            q = 1
            return treasure_is_here


# Поиск пути и его построения
...
print('Done! Check "maze-for-me-done.txt"')