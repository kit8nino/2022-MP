# Класс анализатора сигнала
from scipy.fft import rfft, rfftfreq, irfft
import numpy as np
from gen_simple_signal import Generator_simple

class Analyzer():
    print('Signal --> grafical part')
    
    def __init__(self, signal: Generator_simple):
        self.signal = signal
        self.s = []
    ...
    def plot_fft(self, ax): # построение спектра Фурье из функции fft(self)
        xf, yf = self.fft()
        ax.plot(xf, np.abs(yf))

    
    def plot_rfft(self, ax): # Обратное преобразование Фурье
        xf, yf = self.fft()
        n = len(yf)
